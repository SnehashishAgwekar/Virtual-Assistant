...
function Home() {
  ...
  const speak = (text) => {
    ...
    // fallback — if onend never fires (e.g. new tab opened), restart after 4 seconds
  ...
  const handleCommand = (data) => {
    ...
    if (url) {
      window.open(url, "_blank"); // 👈 open FIRST before speak
    }
  ...
  useEffect(() => {
    ...
    //     console.log("Calling Gemini..."); // ← does it reach here?

    //     isRecognizingRef.current = false;
    //     setListening(false);
    //     const data = await getGeminiResponse(transcript);

    //     console.log("Gemini response:", data); // ← what does it return?
    ...
    recognition.onresult = async (e) => {
      ...
      // ✅ Smart name match that works for ANY name length
      const nameHeard = words.some((word) => {
        if (name.length <= 3) {
          // Short names like "Max", "Sam", "Ali" — must match exactly
          return word === name;
        } else {
          // Longer names — allow 1 char mishear at end
          return word === name || word === name.slice(0, -1);
        }
      });
  ...
  return (
    <div className="w-full h-[100vh] bg-gradient-to-t from-[black] to-[#02023d] flex justify-center items-center flex-col gap-[15px] overflow-hidden">
      ...
      <button
        className="min-w-[150px] h-[60px] mt-[30px] text-black font-semibold bg-white absolute top-[180px] right-[20px] rounded-full cursor-pointer text-[19px] px-[20px] py-[10px] hidden lg:block"
        onClick={() => navigate("/contacts")}
      >
        Manage Contacts
      </button>
